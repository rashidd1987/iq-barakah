...
function renderProgramTasks() {
  ...
  function _updatePtasksHeader() {
    ...
    // Test button — outside container, after calendar btn
    const parent = container.parentElement
    const existingTestBtn = parent.querySelector('.ptasks-test-btn')
  ...
  if (!existing) {
    ...
    calBtn.onclick = () => { ; _addWeekToCalendar(level, weekInLevel, tasks) }
...
function _addWeekToCalendar(level, weekInLevel, tasks) {
  ...
  window.open(`https://calendar.google.com/calendar/render?action=TEMPLATE&text=${title}&dates=${fmtG(monday)}/${fmtG(mondayEnd)}&recur=${recur}&details=${details}&sf=true&output=xml`)
}
