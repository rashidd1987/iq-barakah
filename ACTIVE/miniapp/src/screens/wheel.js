import { haptic, sendData, tg } from '../utils/tg.js'
import { showToast } from '../components/sheets.js'
import { t } from '../i18n.js'

const API_BASE = 'https://pwa-api.iq-barakah.ru'
...
export async function renderWheel() {
  try {
    if (tg?.initData) {
      const response = await fetch(`${API_BASE}/miniapp/wheel`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        if (saved.scores) {
          SECTORS.forEach((sector) => {
            const score = Number(saved.scores[sector.name])
            if (Number.isFinite(score)) sector.val = Math.max(1, Math.min(10, score))
          })
        }
      }
    }
  } catch {
    // Keep the current values when offline. Saving still has Telegram fallback.
  }
  drawWheel()
  buildSliders()
}
...
export async function saveWheel() {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE}/miniapp/wheel/save`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ init_data: tg.initData, scores }),
    })
    if (!response.ok) throw new Error(`Wheel save failed: ${response.status}`)
    showToast(t('wheelSaved'))
  } catch {
    // Older Telegram clients can still persist the wheel through web_app_data.
