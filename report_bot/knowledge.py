...
DEFAULT_BRIEFS = {
    ...
    "mizanos": (
        "Mizan OS — система управления бизнесом собственника: CRM служит "
        "единым центром управления, AI-маршрутизатор — мозгом решений, а "
        "интеграции — нервной системой. Кабинет объединяет деньги, клиентов, "
        "команду, задачи, маркетинг, отчёты, риски и факты выполнения; "
        "AI принимает сигналы сайта, бота, Mini App, оплат, Яндекс.Метрики, "
        "задач и CRM, понимает контекст и рекомендует следующий шаг. Продукт "
        "имеет статус Ready for manual launch. Текущая цель — провести 5–10 "
        "ручных запусков для реальных бизнесов, получить пользователей, "
        "диалоги, заявки, оплаты и кейсы. Новые функции, изменения UI, backend "
        "и product logic запрещены до результатов этих запусков. Публикация "
        "Instagram, обработка Telegram inbox, внешний CRM bridge и MP4 render "
        "пока остаются ручными ограничениями, а не готовой автоматизацией."
    ),
}
            ...
            if isinstance(briefs, dict):
                for key, brief in briefs.items():
                    if isinstance(key, str) and isinstance(brief, str):
                        ...
                        if (
                            key == "mizanos"
                            and normalized == " ".join(LEGACY_MIZANOS_BRIEF.split())
                        ):
                            continue
