...
async def handle_analyze(request):
    ...
    custom_answers = data.get("customAnswers", [])
    ...
    # Собираем свободные ответы пользователя
    custom_lines = []
    for i, block_answers in enumerate(custom_answers):
        if i < 15 and block_answers:
            for ans in block_answers:
                if ans and ans.strip():
                    custom_lines.append(f"  [{blocks[i]}]: «{ans.strip()}»")
    custom_text = "\nСВОИ ОТВЕТЫ (написал сам, не выбрал из списка):\n" + "\n".join(custom_lines) if custom_lines else ""
    ...
    name_call = name if name else "друг"
    prompt = f"""Ты сидишь напротив {name_call} и держишь в руках результаты его диагностики {label}. Это не отчёт для базы данных — это живой разговор с человеком которому ты хочешь помочь по-настоящему.

ЦИФРЫ (средний балл: {avg}%):
{scores_text}{custom_text}

ПРОБОИНЫ (< 40%): {crit_str}
ВНИМАНИЕ (40–69%): {warn_str}
СИЛА (70%+): {strong_str}

Напиши то, что ты бы сказал {name_call} вживую. Структура:

## Что я вижу в твоём корабле
Не сухой анализ — живая картина. Какой образ приходит когда смотришь на эти цифры вместе? Что это напоминает — корабль который вышел в шторм без балласта? Двигатель который работает на 80% из-за одной забитой трубки? Найди точную метафору для ЭТОГО конкретного профиля, не шаблонную.

## То, что ты скорее всего не замечаешь
Самый неожиданный инсайт — не про самый низкий балл, а про связь между зонами. Например: почему высокий {strong[0] if strong else 'один блок'} не спасает от проблем в {crit[0] if crit else 'другом блоке'}. Одно открытие которое меняет всю картину.

## Откуда это всё берётся
Не "недостаточно развито". Честный ответ на вопрос — почему именно так сложилось? Что стоит за этими цифрами по-человечески? Без осуждения, как старший брат который сам через это прошёл.

## Один шаг который изменит всё
{lever_tone}
Назови конкретное действие. Не список — одно. С дедлайном. Такое, которое {name_call} может сделать до конца этой недели.

## Лично от меня
3–4 предложения. Обратись к {name_call} по имени{"" if not name else f" — он это заслуживает"}. Скажи то, что нужно услышать именно ему сейчас. Не мотивашку — настоящие слова. Чтобы он закрыл телефон и почувствовал: меня увидели, в меня верят, я не один на этом пути.

ВАЖНО: пиши от первого лица ("я вижу", "мне кажется", "по-моему"). Никаких "данная диагностика показывает". Никаких списков с цифрами — только живая речь. 350–450 слов."""
