...
async def cmd_start(message: Message, session: AsyncSession, config: Config, state: FSMContext):
    ...
    if payload == "forum_return":
        ...
        await message.answer(
            ...
            f"Ты был одним из первых участников форума IQ Barakah.\n\n"
            f"После форума я многое доработал и теперь хочу показать тебе, что получилось.\n\n"
            f"Прежде чем открыть первый шаг, пройди короткую диагностику.\n\n"
            f"*7 вопросов · около 2 минут*\n\n"
            f"Она поможет определить твою текущую точку, а дальше программа подстроится под тебя.",
            parse_mode="Markdown",
        )
        await message.answer(
            "👇 Нажми чтобы начать:",
