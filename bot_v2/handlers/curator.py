...
async def cmd_addpartner(message: Message, session: AsyncSession, config: Config):
    ...
    if not args:
        ...
        await message.answer(
            f"📋 <b>Скопируй и отправь лидеру:</b>\n\n"
            f"🌿 Ас-саляму алейкум!\n\n"
        ...
        await message.answer(
            f"📋 <b>После прохождения — бот автоматически отправит лидеру:</b>\n\n"
            f"🤝 Теперь — твоя ссылка для аудитории:\n\n"
            f"<code>https://t.me/iqbaraka_bot?start=ref_XXXXX</code>\n\n"
            f"Делись ею с подписчиками — получишь 10% Баракатами с каждой их оплаты автоматически.\n\n"
            f"<i>Чтобы привязать конкретного лидера: /addpartner &lt;user_id&gt;</i>",
            parse_mode="HTML",
        )
        return
    try:
